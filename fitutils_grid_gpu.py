...
def get_grid_estims(grid_preds, grid_space, timeseries_data, gFit, indices, use_gpu=False):
    ...
    if use_gpu:
        ...
        for i in range(nvoxs):
            ...
            # Allocate GPU memory for the timeseries data
            timeseries_data_i = cp.array(timeseries_data[i, :])
            # Allocate GPU memory for the grid predictions
            grid_preds = cp.array(grid_preds)
            # timeseries_data_i = timeseries_data[i, :]
            args = [(timeseries_data_i, grid_preds[j], use_gpu) for j in range(len(grid_preds))]
            rmses = cp.array([compute_rmse(arg) for arg in args])
            best_grid_idx = cp.argmin(rmses)
            best_grid_estim = grid_space[best_grid_idx]
            best_grid_pred = grid_preds[best_grid_idx]
            overload_estim = overload_estimate(best_grid_estim, timeseries_data_i, best_grid_pred, use_gpu)
            # Convert overload_estim to numpy array
            gFit[indices[i], :] = np.array(overload_estim)
            # Clean up GPU memory
